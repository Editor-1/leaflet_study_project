...
export function add2(weatherData) {
  ...
  // 模拟网格数据（实际可以替换为真实数据）
  // const weatherData = Array.from({ length: latGridSize }, () =>
  //   Array.from({ length: lonGridSize }, () => Math.random())
  // );
  ...
  resizedData.forEach((row, y) => {
    row.forEach(([, , value], x) => {
      ...
      // const index = (y * targetSize + x) * 4;
      // const color = Math.floor(v / 40 * 255); // 假设数据范围 [0, 1]
      // imageData.data[index] = color;       // R
      // imageData.data[index + 1] = color;   // G
      // imageData.data[index + 2] = 255 - color; // B
      // imageData.data[index + 3] = 255;     // Alpha
    });
  });
